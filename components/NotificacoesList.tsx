...
import { Bell, Filter, Eye, CheckCircle, AlertTriangle, Info, Building, Store, Heart, FileText, BarChart3, ArrowRight } from 'lucide-react';
...
const expandedNotifications = [
  ...mockNotifications,
  { id: '3', type: 'nova_parceria', title: 'Nova parceria', message: 'Instituto Esperança é parceiro', data: '2024-01-21', read: false, priority: 'media' },
  { id: '4', type: 'nf_pendente', title: 'NF pendente', message: 'NF001234 aguarda processamento', data: '2024-01-20', read: true, priority: 'alta' }
];
...
export function NotificacoesList() {
  ...
  const handleNavigateToSection = (notification) => {
        ...
        <div className="divide-y divide-gray-100">
          {filteredNotifications.map((notification) => {
            ...
            return (
              <div 
                key={notification.id} 
                className={`p-6 hover:bg-gray-50 cursor-pointer ${!notification.read ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`}
                onClick={() => handleNavigateToSection(notification)}
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4 flex-1">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${tipoConf.color.replace('text-', 'text-').replace('bg-', 'bg-')}`}>
                      <TipoIcon size={20} />
                    </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        <Badge className={`text-xs ${tipoConf.color}`}>
                          {tipoConf.label}
                        </Badge>
                        <Badge 
                          variant="outline"
