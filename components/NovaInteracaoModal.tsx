...
import { X, Phone, Mail, MessageSquare, Send, User, Calendar, FileText } from 'lucide-react';
...
const tipoInteracaoConfig = {
  ligacao: { 
    ...
    placeholder: 'Descreva o que foi conversado na ligação: assuntos tratados, acordos feitos, próximos passos...'
  },
  email: { 
    label: 'E-mail', 
    icon: Mail, 
    color: 'bg-purple-100 text-purple-800',
    placeholder: 'Descreva o conteúdo do e-mail: assunto, documentos enviados, solicitações feitas...'
  },
  whatsapp: { 
    label: 'WhatsApp', 
    icon: MessageSquare, 
    color: 'bg-green-100 text-green-800',
    placeholder: 'Descreva a conversa no WhatsApp: mensagens trocadas, combinações feitas...'
  },
  sms: { 
    label: 'SMS', 
    icon: Send, 
    color: 'bg-yellow-100 text-yellow-800',
    placeholder: 'Descreva o conteúdo do SMS: mensagem enviada, resposta recebida...'
  },
  visita: { 
    label: 'Visita', 
    icon: User, 
    color: 'bg-orange-100 text-orange-800',
    placeholder: 'Descreva o que foi discutido na visita: pessoas presentes, pontos abordados, decisões tomadas...'
  }
};
...
export function NovaInteracaoModal({ partner, isOpen, onClose }: NovaInteracaoModalProps) {
  ...
  const tipoConfig = formData.tipo ? tipoInteracaoConfig[formData.tipo] : null;
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              tipoConfig ? tipoConfig.color : 'bg-gray-100'
            }`}>
              {IconComponent ? <IconComponent className="w-5 h-5" /> : <MessageSquare className="w-5 h-5 text-gray-600" />}
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <SelectContent>
                  {Object.entries(tipoInteracaoConfig).map(([key, config]) => {
          ...
          <div>
            ...
            <Textarea 
              ...
              placeholder={tipoConfig?.placeholder || 'Descreva os detalhes da interação...'}
              rows={4}
              required
            />
          </div>
